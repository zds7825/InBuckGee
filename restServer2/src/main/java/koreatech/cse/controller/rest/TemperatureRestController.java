...
public class TemperatureRestController {
    ...
    public ResponseEntity<Temperature> temperature(@PathVariable("sensorId") String sensorId) {
        ...
        if (temperature == null) {  //정보가 없을 때
            System.out.println("Temperature sensor with id (" + sensorId + ") is not found");
            return new ResponseEntity<Temperature>(HttpStatus.NOT_FOUND);   //NOT_FOUND 리턴
        }
        return new ResponseEntity<Temperature>(temperature, HttpStatus.OK); //정보가 있을 때 OK 리턴
    }
